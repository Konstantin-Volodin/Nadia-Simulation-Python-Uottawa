...
def basicColumnsPatientData(df, TF, cutoff):
    df['in_queue'] = df['start'] - df['arrived']
    df['in_system'] = df['end'] - df['arrived']
    df['service_time'] = (df['end'] - df['start']) * 24 * 60 
    if TF:
        df = df[df['arrived'] >= cutoff]
    df = df[df['scan_res'] != ""]
    return df

# Cancer Details
def cancerDetailsAnalysis_Replication(df):
    df_cancer_details = df.groupby(['post_scan_res', 'replication']).id.agg(['count']).reset_index()
    ...
    df_cancer_details['percentage_of_positive_biopsies'] = df_cancer_details['count'] / df_cancer_details[df_cancer_details['post_scan_res'].isin(['Stage_1/2', 'Stage_3/4'])].groupby('replication')['count'].transform('sum') * 100
    df_cancer_details = df_cancer_details[df_cancer_details['post_scan_res'].isin(['Stage_1/2', 'Stage_3/4'])]
    return df_cancer_details
def cancerDetailsAnalysis_Simulation(df_cancer_details):
    df_cancer_details = df_cancer_details.drop(['replication'], axis=1).groupby(['post_scan_res']).agg(['mean', 'std', 'max', 'min', 'median'])
...
def totalPatientDetailsAnalysis_Replication(df):
    ...
    return df_total_unique_arrivals
...
def aggregateUtilizationAnalysis_Replication(df, minutes_array, capacity, repl):
    ...
    df['day'] = np.floor(df['start'])
    df['day_of_week'] = df['day'].mod(7)
    df = df[df['post_scan_res'] != ''].copy()
    new_column = []
    ...
    df = df[['replication', 'day', 'que_to', 'utilization']].groupby(['replication', 'que_to', 'day']).agg(['sum']).reset_index().pipe(flattenMultiIndex)
    df = df.rename(columns={'replication_':'replication','day_':'day','utilization_sum':'utilization', 'que_to_': 'que_to'})

    # Reindex
    min_day = int(df['day'].min())
    max_day = int(df['day'].max())
    new_index = [i for i in range(min_day,max_day+1)]
    df = df.set_index(['day','que_to']).unstack(fill_value=0).reindex(new_index, fill_value=0).stack().sort_index(level=1).reset_index()
    df['day_of_week'] = df['day'].mod(7)
    df = df[df['day_of_week'] != 5]
    df = df[df['day_of_week'] != 6]
    df = df[['replication', 'que_to', 'utilization']]
    df['replication'] = repl

    # Aggregates utilization for replication
    df = df.groupby(['replication','que_to']).agg(['mean', 'max']).reset_index().pipe(flattenMultiIndex) 
    df = df.rename(columns={'replication_':'replication','que_to_':'que_to'})

    return df
def aggregateUtilizationAnalysis_Simulation(df):
    df = df.drop(columns={'replication'}).groupby(['que_to']).agg({
